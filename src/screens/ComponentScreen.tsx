...
const ComponentScreen = ({ navigation, route }: any) => {
  const data = route?.params?.data as ComponentData | undefined;
  const componentItems = Array.isArray(data?.component) ? data.component : [];
    ...
    return (
      <View style={styles.container}>
        ...
        <View style={styles.centerFallback}>
          <Text style={styles.fallbackTitle}>Missing component data</Text>
    ...
    <View style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <View style={styles.headerTitleContainer}>
          <View style={styles.headerIconBadge}>
            <Ionicons name="layers" size={22} color="#1A5276" />
          </View>
          ...
          {componentItems.length > 0 ? (
            componentItems.map((comp, index) => (
              <View key={index} style={styles.tableRow}>
                ...
                <Text style={styles.tableCellDetail}>{comp.detail}</Text>
              </View>
            ))
          ) : (
            <View style={styles.emptyStateRow}>
              <Text style={styles.emptyStateText}>No component details available.</Text>
            </View>
          )}
        </View>
      </ScrollView>
      ...
      <View style={styles.footerSection}>
        <View style={styles.footerBtnRow}>
          <TouchableOpacity
            style={[styles.footerBtn, styles.footerBtnRowItem, { marginRight: 8, opacity: isLoadingIngredients ? 0.7 : 1 }]}
          ...
          >
            {isLoadingIngredients ? (
              <View style={styles.loadingContent}>
                ...
                <Ionicons name="list" size={16} color="#fff" style={styles.loadingIcon} />
                <Text style={styles.footerBtnText}>Please wait</Text>
              </View>
            ) : (
              <View style={styles.buttonContent}>
                <Ionicons name="list" size={16} color="#fff" style={styles.buttonIcon} />
                <Text style={styles.footerBtnText}>View Ingredients</Text>
              </View>
            )}
          </TouchableOpacity>
          <TouchableOpacity
